...
INSERT INTO employees (employee_name, department_id, salary, hire_date) VALUES 
    ...
    SELECT department_name as departments, round(avg(salary)) as avg_salary FROM departments
    JOIN employees USING(department_id)
    GROUP BY department_name


    --Count Employees in Each Department
    SELECT department_name as departments, count(*) FROM departments
    JOIN employees USING(department_id)
    GROUP BY department_name
...
INSERT INTO orders (customer_id, order_date, total_amount) VALUES 
    ...
    SELECT customer_id, count(*) as noOforder, sum(total_amount) totalSpent  FROM orders
    
    GROUP BY customer_id
    HAVING count(order_id)>3

    --Find the total amount of orders placed each month in the year 2022.
    SELECT extract(month from order_date) as order_month, count(*) as number_of_order FROM orders
